...
use crate::animation::model::{
    ...
    AnimationDirectionFrame, AnimationDirectionalCharacter, AnimationGraph, AnimationMaterial,
    AnimationMaterialStyle, AnimationModelProfile, AnimationPathStyle, AnimationPlay,
    AnimationPresent, AnimationProfileRetarget, AnimationRetarget, AnimationRetargetMap,
    AnimationSpritePlayback, AnimationWorld,
};
pub use crate::error::GraphParseError;

const DEFAULT_ANIMATION_DURATION_MS: u64 = 1000;

pub fn is_animation_graph_script(script: &str) -> bool {
    let trimmed = script.trim_start();
    trimmed.starts_with("<Graph")
        && (trimmed.contains("<Animation")
            || trimmed.contains("<World")
            || trimmed.contains("<DirectionalCharacter"))
}

pub fn parse_animation_graph_script(script: &str) -> Result<AnimationGraph, GraphParseError> {
    ...
    if !graph_open.trim_start().starts_with("<Graph") {
        return Err(GraphParseError {
            line,
            message: "Animation DSL must start with <Graph ...>.".to_string(),
        });
    }
    ...
    for block in collect_tag_blocks(graph_inner, "World")? {
        worlds.push(parse_world_block(&block)?);
    }
    for block in collect_tag_blocks(graph_inner, "Animation")? {
        worlds.push(parse_animation_block(
            &block,
    ...
    if worlds.is_empty() {
        return Err(GraphParseError {
            line,
            message: "Animation graph requires at least one <World> or <Animation> block."
                .to_string(),
        });
    }
    ...
    let present_from = if worlds.iter().any(|world| world.id == present_from) {
        ...
        return Err(GraphParseError {
            line,
            message: format!(
                "Present references missing animation world '{}'.",
                present_from
            ),
        });
    };
    let present = AnimationPresent { from: present_from };
    validate_animation_graph_refs(
        &worlds,
        &model_profiles,
        &retargets,
        &actions,
...
fn parse_world_block(block: &str) -> Result<AnimationWorld, GraphParseError> {
    let line = 1;
...
fn parse_animation_block(
    block: &str,
    default_background: Option<AnimationBackground>,
    default_camera: Option<AnimationCamera>,
) -> Result<AnimationWorld, GraphParseError> {
    let line = 1;
    let open_end = block.find('>').ok_or_else(|| GraphParseError {
        line,
        message: "Malformed <Animation> block.".to_string(),
    })?;
    let open = &block[..=open_end];
    let inner = &block[open_end + 1..block.len().saturating_sub("</Animation>".len())];
    let id = required_attr_value(open, "id", line)?;
...
fn parse_directional_characters(
    inner: &str,
) -> Result<Vec<AnimationDirectionalCharacter>, GraphParseError> {
...
fn parse_directional_character_block(
    block: &str,
) -> Result<AnimationDirectionalCharacter, GraphParseError> {
    let line = 1;
    let open_end = block.find('>').ok_or_else(|| GraphParseError {
        line,
    ...
    Ok(AnimationDirectionalCharacter {
...
fn parse_play_sprite_node(block: &str) -> Result<AnimationSpritePlayback, GraphParseError> {
    let line = 1;
...
fn parse_direction_node(block: &str) -> Result<AnimationDirectionFrame, GraphParseError> {
    let line = 1;
...
fn parse_background_node(block: &str) -> Result<AnimationBackground, GraphParseError> {
    let line = 1;
    Ok(AnimationBackground {
...
fn parse_camera_node(block: &str) -> Result<AnimationCamera, GraphParseError> {
    let line = 1;
    let mut camera = AnimationCamera::default();
    camera.id = attr_value(block, "id").or(camera.id);
    camera.mode = attr_value(block, "mode")
        .map(|raw| parse_camera_mode(&raw, line, "mode"))
        .transpose()?
        .unwrap_or(camera.mode);
...
fn parse_actor_node(open: &str, inner: &str) -> Result<AnimationActor, GraphParseError> {
    let line = 1;
...
fn parse_retarget_block(block: &str) -> Result<AnimationRetarget, GraphParseError> {
    let line = 1;
    let open_end = block.find('>').ok_or_else(|| GraphParseError {
        line,
    ...
    for map in collect_self_closing_blocks(inner, "Map")? {
        maps.push(AnimationRetargetMap {
...
fn parse_model_profile_block(block: &str) -> Result<AnimationModelProfile, GraphParseError> {
    let line = 1;
    let open_end = block.find('>').ok_or_else(|| GraphParseError {
        line,
...
fn parse_profile_retarget_block(
    block: &str,
    default_preset: &str,
) -> Result<AnimationProfileRetarget, GraphParseError> {
    let line = 1;
    let open_end = block.find('>').ok_or_else(|| GraphParseError {
        line,
        message: "Malformed <Retarget> block.".to_string(),
    ...
    Ok(AnimationProfileRetarget {
        preset: attr_value(open, "preset").unwrap_or_else(|| default_preset.to_string()),
        maps,
    })
}

fn parse_bone_axis_map_block(block: &str) -> Result<AnimationBoneAxisMap, GraphParseError> {
    let line = 1;
    let open_end = block.find('>').ok_or_else(|| GraphParseError {
        line,
    ...
    Ok(AnimationBoneAxisMap { axes })
}

fn parse_bone_axis_node(block: &str) -> Result<AnimationBoneAxis, GraphParseError> {
    let line = 1;
    Ok(AnimationBoneAxis {
...
fn parse_action_block(block: &str) -> Result<AnimationAction, GraphParseError> {
    let line = 1;
    let open_end = block.find('>').ok_or_else(|| GraphParseError {
        line,
...
fn parse_action_pose_block(block: &str) -> Result<AnimationActionPose, GraphParseError> {
    let line = 1;
    let open_end = block.find('>').ok_or_else(|| GraphParseError {
        line,
...
fn parse_action_bone_node(block: &str) -> Result<AnimationActionBone, GraphParseError> {
    let line = 1;
    Ok(AnimationActionBone {
...
fn parse_apply_action_node(block: &str) -> Result<AnimationApplyAction, GraphParseError> {
    let line = 1;
    Ok(AnimationApplyAction {
...
fn parse_material_node(block: &str) -> Result<AnimationMaterial, GraphParseError> {
    let line = 1;
    Ok(AnimationMaterial {
...
fn parse_play_node(block: &str) -> Result<AnimationPlay, GraphParseError> {
    let line = 1;
    Ok(AnimationPlay {
...
fn validate_animation_graph_refs(
    worlds: &[AnimationWorld],
    model_profiles: &[AnimationModelProfile],
    retargets: &[AnimationRetarget],
    actions: &[AnimationAction],
    apply_actions: &[AnimationApplyAction],
...
fn parse_path_style_attr(block: &str, line: usize) -> Result<AnimationPathStyle, GraphParseError> {
    let Some(raw) = attr_value(block, "pathstyle").or_else(|| attr_value(block, "pathStyle"))
    else {
        return Ok(AnimationPathStyle::Relative);
    };
    match normalize_ident(&raw).as_str() {
        "relative" => Ok(AnimationPathStyle::Relative),
        "absolute" => Ok(AnimationPathStyle::Absolute),
...
fn parse_background_fit(
    ...
    line: usize,
    field: &str,
) -> Result<AnimationBackgroundFit, GraphParseError> {
    match normalize_ident(raw).as_str() {
        "cover" => Ok(AnimationBackgroundFit::Cover),
        "contain" => Ok(AnimationBackgroundFit::Contain),
        "stretch" => Ok(AnimationBackgroundFit::Stretch),
        other => Err(GraphParseError {
            line,
...
fn parse_camera_mode(
    raw: &str,
    line: usize,
    field: &str,
) -> Result<AnimationCameraMode, GraphParseError> {
    match normalize_ident(raw).as_str() {
        "orbit" => Ok(AnimationCameraMode::Orbit),
        "free" => Ok(AnimationCameraMode::Free),
        other => Err(GraphParseError {
            line,
            message: format!("Invalid {field} '{other}'. Expected orbit or free."),
        }),
    }
}
...
fn parse_camera_projection(
    ...
    line: usize,
    field: &str,
) -> Result<AnimationCameraProjection, GraphParseError> {
    match normalize_ident(raw).as_str() {
        "perspective" => Ok(AnimationCameraProjection::Perspective),
        "orthographic" => Ok(AnimationCameraProjection::Orthographic),
        other => Err(GraphParseError {
            line,
...
fn parse_material_style(
    ...
    line: usize,
    field: &str,
) -> Result<AnimationMaterialStyle, GraphParseError> {
    match normalize_ident(raw).as_str() {
        "toon" => Ok(AnimationMaterialStyle::Toon),
        "pbr" => Ok(AnimationMaterialStyle::Pbr),
        "unlit" => Ok(AnimationMaterialStyle::Unlit),
        other => Err(GraphParseError {
            line,
...
#[cfg(test)]
mod tests {
    use crate::animation::AnimationPathStyle;

    use super::parse_animation_graph_script;

    #[test]
    fn parses_basic_animation_graph() {
  ...
  <World id="stage">
    ...
    <Camera id="cam" mode="orbit" target="hero" yaw={curve("0:0:linear,3:360:linear")} pitch="0" distance="3.4" fov="35" />
  ...
  <Present from="stage" />
    ...
    #[test]
    fn parses_no_scope_animation_block_with_top_level_camera() {
        let graph = parse_animation_graph_script(
  ...
  <Animation id="character_animation">
  ...
  <Present from="final" />
        ...
        .expect("animation graph");
        assert_eq!(graph.worlds.len(), 1);
        assert_eq!(graph.worlds[0].id, "character_animation");
        assert_eq!(graph.present.from, "character_animation");
        assert_eq!(
  ...
  <Present from="sprite_stage" />
        ...
        assert_eq!(character.path_style, AnimationPathStyle::Relative);
