...
fn collect_scene_gradient_defs(nodes: &[SceneNode], out: &mut HashMap<String, GradientDef>) {
    for node in nodes {
        match node {
            ...
            SceneNode::Puppet(puppet) => collect_scene_gradient_defs(&puppet.children, out),
...
fn collect_scene_palette_defs(nodes: &[SceneNode], out: &mut HashMap<String, PaletteNode>) {
    for node in nodes {
        match node {
            ...
            SceneNode::Puppet(puppet) => collect_scene_palette_defs(&puppet.children, out),
...
fn collect_scene_texture_defs(nodes: &[SceneNode], out: &mut HashMap<String, TextureDef>) {
    for node in nodes {
        match node {
            ...
            SceneNode::Puppet(puppet) => collect_scene_texture_defs(&puppet.children, out),
...
fn collect_scene_font_defs(nodes: &[SceneNode], out: &mut HashMap<String, FontDef>) {
    for node in nodes {
        match node {
            ...
            SceneNode::Puppet(puppet) => collect_scene_font_defs(&puppet.children, out),
...
fn collect_scene_filter_defs(nodes: &[SceneNode], out: &mut HashMap<String, FilterDef>) {
    for node in nodes {
        match node {
            ...
            SceneNode::Puppet(puppet) => collect_scene_filter_defs(&puppet.children, out),
...
fn collect_scene_component_defs(nodes: &[SceneNode], out: &mut HashMap<String, Vec<SceneNode>>) {
    for node in nodes {
        match node {
            ...
            SceneNode::Puppet(puppet) => collect_scene_component_defs(&puppet.children, out),
...
fn collect_scene_mask_defs(nodes: &[SceneNode], out: &mut HashMap<String, MaskNode>) {
    for node in nodes {
        match node {
            ...
            SceneNode::Puppet(puppet) => collect_scene_mask_defs(&puppet.children, out),
...
fn collect_scene_precompose_defs(nodes: &[SceneNode], out: &mut Vec<PrecomposeNode>) {
    for node in nodes {
        match node {
            ...
            SceneNode::Puppet(puppet) => collect_scene_precompose_defs(&puppet.children, out),
