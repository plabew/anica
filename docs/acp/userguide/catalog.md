...
|---|---|---|---|---|
| UG-0001 | NPM install methods + Codex/Gemini/Claude/OpenCode CLI install and login | User asks how to install npm, install Codex CLI, run `codex login`, use `codex login --device-auth`, install Gemini CLI, login Gemini CLI without API key, setup Claude CLI login, or setup OpenCode ACP. | `npm`, `node`, `install`, `@openai/codex`, `codex login`, `--device-auth`, `@google/gemini-cli`, `gemini`, `/auth`, `oauth-personal`, `claude`, `claude auth login`, `claude auth status`, `@anthropic-ai/claude-code`, `opencode`, `opencode-ai`, `opencode auth login`, `opencode acp`, `anica-acp proxy`, `configOptions`, `session/set_config_option`, `OpenCode model`, `global install` | `anica/docs/acp/userguide/UG-0001.md` |
